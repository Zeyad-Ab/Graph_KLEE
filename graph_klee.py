...
class KLEEGraphParser:
    ...
    def __init__(self, neo4j_uri: str = "bolt://localhost:7687", 
                 neo4j_user: str = "neo4j", neo4j_password: str = "@leoleoleo",
    ...
    def _parse_error_files(self, klee_path: Path) -> List[MemoryError]:
        ...
        for error_file in error_files:
            test_id = error_file.stem.replace('.read_only', '').replace('.out_of_bound', '').replace('.external', '').replace('.abort', '').replace('.assert', '')
...
def main():
    ...
    parser = KLEEGraphParser(
        ...
        neo4j_password=os.getenv("NEO4J_PASSWORD", "@leoleoleo"),
        neo4j_database=os.getenv("NEO4J_DATABASE", "Klee_Graph")
    )
